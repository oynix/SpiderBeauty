...
do
	...
	fi
	{
		...
		if [[ ! -s $article_cut_file ]]; then
			temp1="${dir}/.temp1"
			temp2="${dir}/.temp2"
			sed -n -e "s/article-content/&\n/g;w $temp1" $article_file
			sed -n -e "1,/article-content/d;w $temp2" $temp1
			sed -n -e "s/excerpts-article/\n&/g;w $temp1" $temp2
			sed -n -e "/excerpts-article/,\$d;w $temp2" $temp1
			cat $temp2 | tr -d '\n' > $temp1
			sed -n -e "s/<img/\n&/g;w $temp2" $temp1
			sed -n -e "/<img/!d;w $article_cut_file" $temp2
			rm $temp1
			rm $temp2
		fi
