...
do
	...
	fi
	{
		...
		if [[ ! -s $article_file ]]; then
			...
			ret=$?
			if [[ $ret != 0 ]]; then
				echo "\033[31mRequest Error: ret=$ret, index=${index}, url=$article_url \033[0m"
		...
		if [[ ! -s $article_cut_file ]]; then
			temp1="${dir}/.temp1"
			temp2="${dir}/.temp2"
			LC_ALL=C sed -n -e "s/article-content/\n&\n/g;w $temp1" $article_file
			LC_ALL=C sed -n -e "1,/article-content/d;w $temp2" $temp1
			LC_ALL=C sed -n -e "s/<\/div>/\n&\n/1;w $temp1" $temp2
			LC_ALL=C sed -n -e "/<\/div>/,\$d;w $temp2" $temp1
			cat $temp2 | tr -d '\n' | sed -n -e "s/<img/\n&/g;w $temp1"
			LC_ALL=C sed -n -e "/<img/!d;w $article_cut_file" $temp1
			if [[ ! -s $temp1 || ! -s $temp2 ]]; then
				rm $article_file
				echo "\033[31mCur Error: index=${index} dir=${dir} url=${article_url} \033[0m"
				echo >&9
				exit
			fi
			rm $temp1
